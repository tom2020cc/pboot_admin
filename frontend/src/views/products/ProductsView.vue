      ...
      <el-table-column label="翻译进度" width="180" align="center">
        <template #default="{ row }">
          <el-tooltip v-if="row.translationProgress" placement="top" :disabled="!row.translationProgress.missing.length">
            ...
            <div class="product-translation-status" tabindex="0">
...
import ProductScopeSyncDialog from "@/components/ContentScopeSyncDialog.vue";
...
const handleSync = async (id: number) => {
  syncingId.value = id;
  try {
    const res = await syncProductToPboot(id, { all: true });
    await showProductSyncResult(res.data, async () => (await syncProductToPboot(id, { all: true })).data);
  } catch (e) {
...
.product-translation-status { display: flex; flex-direction: column; align-items: center; gap: 4px; }
.product-translation-status span:last-child { font-size: 12px; }
...
@media (max-width: 900px) {
  .page-bar, .page-actions { flex-wrap: wrap; }
