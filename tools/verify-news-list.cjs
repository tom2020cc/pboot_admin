...
const output = path.resolve(__dirname, '../tmp/news-list-check');
const languages = ['zh-CN', 'en', 'es', 'fr', 'ru', 'ar', 'pt', 'id', 'tr', 'vi'];
const articles = ['complete', 'partial', 'untranslated'].map((status, index) => ({
  id: index + 1, title: `新闻${index + 1}`, menuId: 1, show: true, orderNum: index,
  carouselImages: [], thumbnail: '', lang: 'zh-CN',
  translationProgress: { status, total: 9, completed: status === 'complete' ? 9 : status === 'partial' ? 6 : 0,
    completedLanguages: [], missing: status === 'complete' ? [] : (status === 'partial' ? ['id', 'tr', 'vi'] : languages.slice(1)).map(lang => ({ lang, fields: ['正文'] })) },
}));
const menus = [
  { id: 1, parentId: 0, code: 'pboot:cn:1', name: '新闻中心', href: '/rigs', model: '2' },
  { id: 2, parentId: 1, code: 'pboot:cn:2', name: '公司新闻', href: '/core', model: '2' },
  { id: 3, parentId: 0, code: 'pboot:en:3', name: 'News', href: '/en-rigs', model: '2' },
];
const preview = { siteId: 2, siteName: '测试项目', menuId: 1, menuName: '新闻中心', totalNews: 3,
  totalLanguages: 10, readyCount: 18, skipped: [{ newsId: 2, title: '新闻2', lang: 'vi', reason: '缺少正文' }], blocked: [],
...
(async () => {
  ...
  try {
    ...
    await page.route('http://localhost:5108/**', async route => {
      ...
      else if (url.pathname === '/news') data = articles;
      else if (url.pathname === '/news/pboot-stats') data = { localCount: 3, syncCount: 0, diff: -3 };
      else if (url.pathname === '/news/translation-models') data = [];
      else if (url.pathname === '/news/pboot-scope/preview-all') {
        assert.equal(req.postDataJSON().menuId > 0, true);
        data = { ...preview, blocked: blocked ? [{ newsId: 1, title: '新闻1', lang: 'en', reason: '对应栏目尚未写入 PB' }] : [] };
      } else if (url.pathname === '/news/pboot-scope/sync-all') {
    ...
    await page.goto(process.env.NEWS_LIST_URL || 'http://localhost:5278/#/news');
    await page.getByText('部分未翻译', { exact: true }).waitFor();
    assert.equal(await page.locator('.news-translation-status').count(), 3);
    await page.getByText('部分未翻译', { exact: true }).hover();
    await page.getByText('Bahasa Indonesia：缺少正文').waitFor();
    await page.locator('.progress-select').click();
    await page.getByRole('option', { name: '未完成翻译', exact: true }).click();
    assert.equal(await page.locator('.news-translation-status').count(), 2);
    await page.locator('.progress-select').click();
    await page.getByRole('option', { name: '已全部翻译', exact: true }).click();
    assert.equal(await page.locator('.news-translation-status').count(), 1);
    await page.locator('.progress-select').click();
    ...
    assert.equal(await options.filter({ hasText: 'News' }).count(), 0);
    await page.getByRole('option', { name: '新闻中心', exact: true }).click();
    await dialog.getByText('个可同步版本').waitFor();
    await dialog.getByText('跳过 1 个内容未完整的语言版本').click();
    await dialog.getByText('新闻2 / Tiếng Việt：缺少正文').waitFor();
    await page.waitForFunction(() => document.getAnimations().every(animation => animation.playState !== 'running'));
