...
- `managed-sites/<站点标识>/seo/seo.config.json` 保存当前网站路径、公开域名和搜索引擎配置。
- AI Key、FTP 密码等敏感配置不要放进公共压缩包。
- 迁移到新电脑时导入整套配置备份；新增网站时在集中“站点管理”中扫描或添加。
- 数据库备份目录不要上传到公开网站。

## 适配其他 PbootCMS 网站

多个网站共用一份管理工具和一组端口，站点差异由 `siteId` 和 `managed-sites/<站点标识>/` 隔离。

迁移到新网站后按以下顺序操作：

1. 在管理后台顶部选择目标网站；没有时先到“站点管理”扫描添加。
2. 核对该网站根目录、`data` 目录中的 SQLite 数据库和真实线上地址。
3. 配置该网站自己的 FTP、Google、IndexNow 和百度参数。
4. 启动后先核对 SEO 页面显示的当前站点；任何一项仍显示旧网站时立即停止操作并检查站点选择。
5. 点击“一键准备 Google Sitemap”，确认生成文件、FTP 上传和线上检查都针对新域名。
6. 在新域名自己的 Search Console 资源中提交 Sitemap。

不要为每个网站分配新端口。后台、SEO 和 FTP 页面会把当前 `siteId` 传给接口；即使同时打开两个网站的工具标签页，API 请求也会保持各自站点上下文。
